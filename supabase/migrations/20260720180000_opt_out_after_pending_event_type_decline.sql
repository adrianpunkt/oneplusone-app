-- A pending member who says the event type is not interesting is opting out
-- of future event invitations, not only declining this particular event.
...
join public.event_invitations as invitations
  on invitations.id = declines.invitation_id
where declines.reason = 'event_type_not_interested'
  and invitations.member_status_at_invite = 'pending'
